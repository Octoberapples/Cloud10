...
def dat2gmsh(x,y):
  ...
  output = ""
  while i < x.size:
    output +=  "Point(" + str(i+1) + ") = {" + str(x[i]) + "," + str(y[i])+",0,"+str(lc1)+"};\n"
    i += 1 
  ntot = x.size
  i = 1
  while i < ntot:
    output +=  "Line(" + str(i) + ")={" + str(i) + "," + str(i+1) +"};\n"
    i += 1
  output +=  "Line(" + str(ntot) + ")={" + str(ntot) + "," + "1};\n"
  output +=  "Line Loop(" + str(ntot+1) + ")={\n"# % (ntot+1),
  i = 1
  while i < ntot:
    output += ""+str(i)+", "
    i += 1 
...
# Outer domain boundary
  output +=  "Point(100000) = {-10,0,0,"+str(lc2)+"};\n"
  output +=  "Point(101000) = {0,10,0,"+str(lc2)+"};\n"
  output +=  "Point(102000) = {10,0,0,"+str(lc2)+"};\n"
  output +=  "Point(103000) = {0,-10,0,"+str(lc2)+"};\n"
  output +=  "Point(104000) = {0,0,0,"+str(lc2)+"};\n"
  output +=  "Circle(105000) = {100000,104000,101000};\n"
  output +=  "Circle(106000) = {101000,104000,102000};\n"
  output +=  "Circle(107000) = {102000,104000,103000};\n"
  output +=  "Circle(108000) = {103000,104000,100000};\n"
  output +=  "Line Loop(109000) = {105000,106000,107000,108000};\n"
  output +=  "Plane Surface(110000) = {109000,"+str(ntot+1)+"};\n"
...
# Main loop
# Args: 
# n1, n2, n3, n4, ange - float
# n_nodes - int
###########################################################################

def main(n1, n2, n3, n4, angle, n_nodes):
  xs = np.linspace(0.0,1.0,n_nodes)
  x, y = naca4(n1,n2,n3,n4,xs)
  xa, ya = rot(x,y,angle)
  return dat2gmsh(xa,ya)
###########################################################################
